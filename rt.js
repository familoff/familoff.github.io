	...
	function rating_kp_imdb(card) {
		...
		function _getCache(movie) {
			...
			if (cache[movie]) {
				if ((timestamp - cache[movie].timestamp) > params.cache_time) {
                    ...
                    Lampa.Storage.set('tomatoes_rating', cache);
					return false;
				}
			} else return false;
			return cache;
		...
		function _showRating(data) {
			if (data) {
                ...
                var tomatoes_rating = !isNaN(data.imdb) && data.imdb !== null ? parseFloat(data.imdb).toFixed(1) : '0.0';
                ...
                $('.rate--tomatoes', render).removeClass('hide').find('> div').eq(0).text(tomatoes_rating);
			}
		}
	}
